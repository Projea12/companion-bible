...
impl AssemblyAiTranscriber {
    ...
    pub fn start(&mut self) {
        ...
        self.handle = Some(tokio::spawn(async move {
            ...
            loop {
                ...
                .await
                {
                    Ok(()) => {
                        ...
                        eprintln!("[assemblyai] stream closed — reconnecting");
                    }
                    Err(e) => {
                        if stop_flag.load(Ordering::Acquire) {
                            break;
                        ...
                        eprintln!(
                            "[assemblyai] stream error: {e} — reconnecting in {backoff_secs}s"
                        );
                    }
                }
            ...
            eprintln!("[assemblyai] supervisor stopped");
        }));
    }
...
async fn stream_loop(
    ...
    // Wait up to 2 s for the audio task to send its Terminate frame before
...
mod tests {
    ...
    async fn stop_flag_exits_audio_loop_within_timeout_window() {
        ...
        let mut handle = tokio::spawn(async move {
            ...
            // Simulate the Terminate send (fast I/O, ≤10 ms in practice).
            tokio::time::sleep(Duration::from_millis(10)).await;
        });
